...
type Entry = typeof snippets.$inferSelect;

function getPending(): Entry[] {
  return db.select().from(snippets).where(eq(snippets.pendingSync, 1)).all();
}

function markPushed(pushed: Entry, remoteId: string, serverUpdatedAt: string): void {
  const unchanged = and(
    eq(snippets.trigger, pushed.trigger),
    eq(snippets.replacement, pushed.replacement),
    isNull(snippets.deletedAt),
    sql`${snippets.updatedAt} IS ${pushed.updatedAt}`,
  );
  db.update(snippets)
    .set({
      remoteId,
      pendingSync: sql`CASE WHEN ${unchanged} THEN 0 ELSE ${snippets.pendingSync} END`,
      updatedAt: sql`CASE WHEN ${unchanged} THEN ${serverUpdatedAt} ELSE ${snippets.updatedAt} END`,
    })
    .where(and(eq(snippets.id, pushed.id), eq(snippets.clientSnippetId, pushed.clientSnippetId!)))
    .run();
}
...
function markTerminal(localId: number): void {
  ...
  db.update(snippets).set({ pendingSync: 0 }).where(eq(snippets.id, localId)).run();
}
...
function clearRemoteId(localId: number): void {
  db.update(snippets).set({ remoteId: null }).where(eq(snippets.id, localId)).run();
}

function hardDelete(localId: number): void {
  db.delete(snippets).where(eq(snippets.id, localId)).run();
}

export async function pushSnippets(checkpoint: SyncCheckpoint = uncheckedSync): Promise<void> {
  checkpoint();
  const pending = getPending();
  if (pending.length === 0) return;

  const creates: { pushed: Entry; localId: number; payload: SnippetPushInput }[] = [];
  const updates: {
    pushed: Entry;
    localId: number;
    remoteId: string;
    payload: { trigger: string; replacement: string };
  }[] = [];
  ...
  for (const entry of pending) {
    ...
    if (!entry.clientSnippetId) {
      Sentry.captureMessage('pushSnippets: missing clientSnippetId', 'warning');
    ...
    if (!entry.remoteId) {
      creates.push({
        ...
        localId: entry.id,
        payload: {
          client_snippet_id: entry.clientSnippetId,
    ...
    updates.push({
      ...
      payload: { trigger: entry.trigger, replacement: entry.replacement },
    });
  }
  ...
  for (let i = 0; i < creates.length; i += PUSH_BATCH_SIZE) {
    ...
    try {
      checkpoint(true);
      const created = await batchCreateSnippets(chunk.map((c) => c.payload));
      checkpoint();
      // Server may dedupe on lower(trigger); match by client_snippet_id (its
      // upsert favors the incoming client_snippet_id, so the returned row should
      // carry the one we sent).
      const byClientId = new Map(created.map((c) => [c.client_snippet_id, c]));
      let unmatched = 0;
      for (const c of chunk) {
        const server = byClientId.get(c.payload.client_snippet_id);
        if (server) {
      ...
      if (unmatched > 0) {
        Sentry.captureMessage(
          `pushSnippets: ${unmatched}/${chunk.length} create rows had no matching server response`,
          'warning',
        );
      }
    } catch (e) {
      ...
      if (isTerminal(e)) {
        ...
        Sentry.captureException(e, { tags: { sync: 'pushSnippets.create.terminal' } });
      } else {
        failed += 1;
        Sentry.captureException(e, { tags: { sync: 'pushSnippets.create' } });
      }
    }
  }
  ...
  for (const u of updates) {
    try {
      checkpoint(true);
      const server = await updateSnippet(u.remoteId, u.payload);
      checkpoint();
      if (!server || !server.id) {
        Sentry.captureMessage('pushSnippets: update returned empty response', 'warning');
      ...
      if (isHttpStatus(e, 404)) {
        ...
        Sentry.captureException(e, { tags: { sync: 'pushSnippets.update.404' } });
      } else if (isTerminal(e)) {
        markTerminal(u.localId);
        Sentry.captureException(e, { tags: { sync: 'pushSnippets.update.terminal' } });
      } else {
        failed += 1;
        Sentry.captureException(e, { tags: { sync: 'pushSnippets.update' } });
      }
    }
  }
  ...
  for (const d of deletes) {
    try {
      checkpoint(true);
      await deleteSnippet(d.remoteId);
      checkpoint();
      ...
      if (isHttpStatus(e, 404)) {
        ...
        Sentry.captureException(e, { tags: { sync: 'pushSnippets.delete.terminal' } });
      } else {
        failed += 1;
        Sentry.captureException(e, { tags: { sync: 'pushSnippets.delete' } });
      }
    }
  }

  if (failed > 0) {
    throw new Error(`pushSnippets: ${failed} operation(s) failed`);
  }
}
